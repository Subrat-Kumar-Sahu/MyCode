...
def search_col(p_db, p_table, p_search_str):
    trg_db = p_db
    trg_table = p_table
    search_str = p_search_str
    ...
    if REC_COUNT > 0:
        for i in col_list:
            ...
            if REC_KOUNT > 0:
                print("{0}:{1}:{2}:{3}".format(trg_db, trg_table, i, search_str))
    else:
        print("String '{0}' not found in {1}.{2}".format(search_str, trg_db, trg_table))
...
if __name__ == '__main__':
    ...
    print("Argument Details: {0}".format(args))

    if not args.db.strip() or not args.table.strip() or not args.search.strip():
        print("None of the argument can be blank/null")
    else:
